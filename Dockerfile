...
# 2. Install dependencies WITHOUT the autoloader first to save time
RUN composer install --no-dev --ignore-platform-reqs --no-interaction --no-autoloader

# 3. Now copy the WHOLE project code
COPY . .

# 4. Finally, generate the "Optimized Autoloader" now that all files exist
